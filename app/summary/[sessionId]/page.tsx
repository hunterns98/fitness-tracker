...
type SetRow = {
  ...
  rpe: number | null
  note: string | null
  exercise: { name: string; target_reps: string | null }
}
  ...
  return (
    <div className="flex-1">
      ...
      <div className="flex items-center gap-1.5">
        <button
          className="step-btn !w-8 !h-8 text-base"
...
export default function SummaryPage() {
      ...
      .then(({ session: s, sets }) => {
        ...
        for (const row of (sets as SetRow[])) {
          if (!byEx[row.exercise_id]) {
            byEx[row.exercise_id] = { exercise_id: row.exercise_id, name: row.exercise?.name ?? '—', sets: [], target_reps: row.exercise?.target_reps ?? null }
          }
  ...
  function startEditSet(s: SetRow) {
    ...
    setEditValues({ reps: s.reps, weight_kg: s.weight_kg, rpe: s.rpe, note: s.note ?? '' })
  }
  ...
  async function saveEditSet(s: SetRow, gi: number) {
    ...
    const res = await fetch('/api/sets', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: sessionId, exercise_id: s.exercise_id, set_number: s.set_number, reps: editValues.reps, weight_kg: editValues.weight_kg, rpe: editValues.rpe, note: editValues.note || null }),
    })
    const updated = await res.json()
    setGroups(prev => prev.map((g, idx) => idx !== gi ? g : {
      ...g, sets: g.sets.map(row => row.id === s.id ? { ...row, ...updated } : row)
    }))
  ...
  function parseMaxReps(t: string | null) {
    const m = t?.match(/\d+/g)
    return m ? parseInt(m[m.length - 1]) : 12
  }

  if (loading) {
    return <div className="flex items-center justify-center h-screen"><p className="text-gray-500 text-sm">Đang tải...</p></div>
  }
    ...
    <div className="px-4 pt-6 pb-20 space-y-5">
        ...
        <section>
          ...
          {editingRun && (
            <div className="card p-4 space-y-4">
              ...
              <Stepper label="Calo" value={runForm.calories} step={10} onChange={v => setRunForm(f => ({ ...f, calories: v }))} />
      ...
      {!isRun && groups.length > 0 && (
        <section className="space-y-3">
          ...
          {groups.map((g, gi) => {
            const canUp = shouldIncreaseWeight(g.sets, parseMaxReps(g.target_reps))
            return (
                      ...
                      <div key={s.id}>
                        {!isEditThis ? (
                          <div className="flex items-center gap-2 text-xs text-gray-400">
                            ...
                            <button onClick={() => startEditSet(s)} className="ml-auto border border-gray-800 px-1.5 py-0.5 rounded text-gray-700">Sửa</button>
                          </div>
                        ) : (
                          <div className="bg-gray-800 rounded-xl p-3 space-y-2.5 mt-1">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2">
                                <button className="step-btn !w-8 !h-8"
                                  onPointerDown={e => { e.preventDefault(); setEditValues(ev => ({ ...ev, rpe: Math.max(6, Math.round(((ev.rpe ?? 7) - 0.5) * 10) / 10) })) }}>−</button>
                                ...
                                <button className="step-btn !w-8 !h-8"
                                  onPointerDown={e => { e.preventDefault(); setEditValues(ev => ({ ...ev, rpe: Math.min(10, Math.round(((ev.rpe ?? 6.5) + 0.5) * 10) / 10) })) }}>+</button>
                              </div>
                            </div>
                            <input type="text" value={editValues.note}
                              onChange={e => setEditValues(ev => ({ ...ev, note: e.target.value }))}
                              placeholder="Ghi chú..." className="w-full px-3 py-1.5 rounded-xl bg-gray-700 text-sm text-gray-300 outline-none" />
                            <div className="flex gap-2">
                              <button onClick={() => saveEditSet(s, gi)} disabled={savingSet} className="flex-1 btn-primary py-2 text-sm">
                                {savingSet ? 'Đang lưu...' : 'Lưu'}
                              </button>
